                            ...
                            child: Column(
                              children: [
                                if (chat.filePath != null)
                                  GestureDetector(
                                    onTap: () => OpenFile.open(chat.filePath!),
                                    child: Image.file(
                                      File(chat.filePath!),
                                      height: 200,
                                    ),
                      ...
                      : IconButton(
                          ...
                          onPressed: () async {
                            ...
                            if (text.isNotEmpty) {
                              ...
                              if (filePath != null) {
                                final newFile =
                                    await imageController.saveFile(filePath);
                                imageController.addMessage(ChatImage(
                                    role: 'user',
                                    part: text,
                                    filePath: newFile.path));
                                pathStr = newFile;
                              } else {
                                imageController.addMessage(
                                    ChatImage(role: 'user', part: text));
                              }
                              imageController
                                  .addMessage(ChatImage(role: 'bot', part: ''));
                              imageController.scrolldown();
                              _controller.clear();
                              final data = await imageController.getMessage(
                                  text, pathStr);
                              imageController.lastMessage();
                              imageController.addMessage(
                                  ChatImage(role: 'bot', part: data!));
                              imageController.scrolldown();
                              setState(() {
                                hasPicked = false;
                                isLoading = false;
