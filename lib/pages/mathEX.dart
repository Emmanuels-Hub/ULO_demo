                      ...
                      : IconButton(
                          ...
                          onPressed: () async {
                            ...
                            if (text.isNotEmpty) {
                              ...
                              if (filePath != null) {
                                final newFile =
                                    await mathController.saveFile(filePath);
                                mathController.addMessage(ChatGeometry(
                                    role: 'user',
                                    part: text,
                                    filePath: newFile.path));
                                pathStr = newFile;
                              } else {
                                mathController.addMessage(
                                    ChatGeometry(role: 'user', part: text));
                              }
                              mathController.addMessage(
                                  ChatGeometry(role: 'bot', part: ''));
                              mathController.scrolldown();
                              _controller.clear();
                              final data = await mathController.getMessage(
                                  text, pathStr);
                              mathController.lastMessage();
                              mathController.addMessage(
                                  ChatGeometry(role: 'bot', part: data!));
                              mathController.scrolldown();
                              setState(() {
                                hasPicked = false;
                                isLoading = false;
